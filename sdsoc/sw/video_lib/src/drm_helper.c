...
static int drm_find_crtc(struct drm_device *dev)
{
	...
	vlib_dbg("res->count_connectors = %d\n", res->count_connectors);
	...
	for (size_t i=0; i<dev->connector->count_modes; i++) {
		vlib_dbg("mode %ld :: hdisplay %u ::  vdisplay %u :: vrefresh %u\n", i,
				dev->connector->modes[i].hdisplay, 
				dev->connector->modes[i].vdisplay,
				dev->connector->modes[i].vrefresh);	
	}
...
static int drm_find_plane(struct drm_device *dev, struct vlib_plane *p)
{
	...
	vlib_dbg("\tprimary crtc id: %d\n", dev->prim_plane.drm_plane->crtc_id);
	vlib_dbg("\tprimary plane id: %d\n", dev->prim_plane.drm_plane->plane_id);
	vlib_dbg("\tprimary plane format: %d\n", dev->prim_plane.drm_plane->formats[0]);

	if (dev->overlay_plane.drm_plane) {
		vlib_dbg("\toverlay plane crtc id: %d\n", dev->overlay_plane.drm_plane->crtc_id);
		vlib_dbg("\toverlay plane id: %d\n", dev->overlay_plane.drm_plane->plane_id);
		vlib_dbg("\toverlay plane format: %d\n", dev->overlay_plane.drm_plane->formats[0]);
	}
...
int drm_set_plane(struct drm_device *dev, int index)
{
	...
	return drmModeSetPlane(dev->fd, dev->overlay_plane.drm_plane->plane_id,
			...
			dev->overlay_plane.vlib_plane.xoffs, /* crtc_x */
...
void drm_uninit (struct drm_device *dev)
{
	...
	if (dev->prim_plane.drm_plane->plane_id == dev->overlay_plane.drm_plane->plane_id)
	{
...
int drm_find_preferred_mode(struct drm_device *dev)
{
	...
	/* First avertised mode is preferred mode */
